...
try {
    ...
    if ($volumeStatus -eq 'FullyEncrypted') {
        ...
        if ($hasRecovery -and $hasTpm -and -not $hasTpmPin) {
            ...
            Write-Log "Suspending BitLocker to force TPM reseal..."
            Suspend-BitLocker -MountPoint $MountPoint -RebootCount 0 -Verbose
            Write-Log "Resuming BitLocker to bind to current system PCR measurements..."
            Resume-BitLocker -MountPoint $MountPoint -Verbose
